...
@router.get("/")
async def settings_page(request: Request, user: dict = Depends(require_role("super_admin"))):
    db = request.state.db
...
async def save_settings(request: Request, user: dict = Depends(require_role("super_admin"))):
    ...
    temp_rules_value = get_or_create_setting(db, "temp_parking_rules", '{"free_minutes": 30, "daily_cap": 20.00, "tiers": [], "overflow": "daily_reset"}').value
    try:
        temp_rules_data = json.loads(temp_rules_value)
    except json.JSONDecodeError:
        temp_rules_data = {"free_minutes": 30, "daily_cap": 20.00, "tiers": [], "overflow": "daily_reset"}
...
async def save_temp_rules(request: Request, user: dict = Depends(require_role("super_admin"))):
    ...
    return templates.TemplateResponse("settings/form.html", {
        ...
        "overflow_options": ["daily_reset", "continue"],
        "area_options": get_or_create_setting(db, "area_options", "A区,B区,C区,D区"),
        "room_format_patterns": get_or_create_setting(db, "room_format_patterns", "A区1幢2单元301室,A区1幢301室,1幢2单元301室,1幢301室"),
        "success": "临时停车规则已更新"
    })
